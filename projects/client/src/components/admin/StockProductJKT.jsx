...
import CreateStockJKT from "./CreateStockJKT";

const StockProductJKT = () => {
  ...
  useEffect(() => {
    fetch(
      ...
      .then((data) => {
        const jabodetabekStock = data.data.filter(
          (stock) => stock.Branch.id === 2
        );
        setStockData(jabodetabekStock);
      })
      .then((data) => {
  ...
  return (
    <Box p={4}>
      ...
      <Box mt={1}>
        {stockData.length === 0 ? (
          <Box>No stock records found for Jabodetabek.</Box>
        ) : (
          <Table
          ...
          >
            <TableCaption>Stock data for Jabodetabek</TableCaption>
...
export default StockProductJKT;
